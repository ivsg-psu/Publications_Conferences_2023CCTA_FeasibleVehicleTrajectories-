%%%%%%%%%% script_createRealisticTrajectoriesSCE_7DoFPPC.m %%%%%%%%%%
%% Purpose:
% The pupose of this script create feasible reference trajectories to
%   estimate friction demand by letting a 7DOF vehicle model track the
...
controller.velocity_Pgain      = 200; % P gain for velocity control
...
for index_sectionID = sectionId
    ...
    for index_vehicle = vehicle_ID
            ...
            for i = 1:N_laneChanges % loop through all lane changes that occur
                ...
                if 20 < dist_to_path_start && 30 < dist_to_path_end
                    ...
                    if recalculate_lane_length
                        ...
                        % Redecimate the LCL at a station interval that will give you the
                        %   same number of elements as the reference trajectory stations
                        station_interval = start_lane.Station(end)/length(RT_veh_station);
                    ...
                    % find the indices for before the LC happnes
        ...
        if stop_traj_sim == false% Indices where the vehicle stopped
            ...
            end % NOTE: END IF statement for flag.plot
        end % NOTE: END IF statement for stop_traj_sim
